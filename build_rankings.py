...
def build():
    ...
    if missing:
        # 우리가 관심 있는 회사 접두사 — 이 목록은 필요하면 자유롭게 늘려도 됨
        INTEREST_PREFIXES = ("claude-", "gpt-", "gemini-", "kimi-", "qwen",
                             "minimax-", "gemma-", "grok-", "llama-", "deepseek-",
                             "mistral-", "nano-banana", "flux", "seedream")
        interest = sorted([m for m in missing if m.lower().startswith(INTEREST_PREFIXES)])
        others   = sorted([m for m in missing if not m.lower().startswith(INTEREST_PREFIXES)])

        if interest:
            print(f"  ⚠ 매핑 없음(주요 회사, 검토 필요) {len(interest)}개:")
            for name in interest:
                print(f"      - {name}")
        if others:
            print(f"  ℹ 매핑 없음(기타, 대체로 무시 가능): {len(others)}개")
            # 기타는 처음 몇 개만 예시로 (스팸 방지)
            for name in others[:5]:
                print(f"      - {name}")
            if len(others) > 5:
                print(f"      ... 외 {len(others)-5}개")
    if stale:
